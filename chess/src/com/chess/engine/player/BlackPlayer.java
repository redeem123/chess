		...
		if (this.playerKing.isFirstMove() && !this.isInCheck()) {
			...
			if (!this.board.getTile(5).isTileOccupied() && !this.board.getTile(6).isTileOccupied()) {
				...
				if (rookTile.isTileOccupied() && rookTile.getPiece().isFirstMove()) {
					if (Player.calculateAttackOnTile(5, opponentLegals).isEmpty()
						...
						kingCastles.add(new Move.KingSideCastleMove(board, this.playerKing, 6,
			...
			if (this.board.getTile(1).isTileOccupied() && !this.board.getTile(2).isTileOccupied()
				...
				if (rookTile.isTileOccupied() && rookTile.getPiece().isFirstMove()
					...
					kingCastles.add(new Move.QueenSideCastleMove(board, this.playerKing, 2, (Rook) rookTile.getPiece(),
